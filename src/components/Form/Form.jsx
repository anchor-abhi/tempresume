...
const courseTitleLabel =
	"Course/Degree Title  (maximum " + courseTitleMaxLength + " characters) *";
const collegeTitleLabel =
	"College/institute/School name (maximum " +
	collegeTitleMaxLength +
	" characters) *";

const projectTitleLabel =
	"Project Title  (maximum " + projectTitleMaxLength + " characters) *";
const projectIntroLabel =
	"Write a quick brief about project (maximum " +
	projectIntroMaxLength +
	" characters) *";
const projectRolesLabel =
	"Project Roles, each in new line (maximum " +
	projectRolesMaxLength +
	" characters) *";
const projectFeaturesLabel =
	"Project Features, each in new line (maximum " +
	projectFeaturesMaxLength +
	" characters) *";
...
const Form = () => {
			...
			axios
				...
				.then((res) => {
					...
					if (res.data.length == 0) {
						...
						var tempLinkedin =
							res.data[dataLength].personal.linkedin.split("/");
						...
						setStudentAccomplishment(
							res.data[dataLength].accomplishments.join("\n")
						);
						setStudentInterests(res.data[dataLength].interests.join(", "));

						if (
							res.data[dataLength].workEx &&
							res.data[dataLength].workEx.length != 0
						) {
	...
	const addProject = () => {
		...
		if (prFeatures.length > 3) {
			setProjectFeaturesError(
				"Maximum you can add 3 points in features of Project"
			);
	...
	const addExperience = () => {
		...
		if (workRolesArray.length > 3) {
			setWorkRolesError(
				"Maximum you can add 3 points in work experience roles"
			);
			return;
		}
		setOpenExperienceForm(false);
				...
				<div className="form-container">
					<div className="header-section">
						<div className="proflile-img">
							...
							<label for="upload">
								Upload Avatar
								<input type="file" id="upload" onChange={onSelectFile} />
							</label>
            ...
            <input type="file" onChange={onSelectFile} accept="image/png, image/jpeg" className="custom-file-input" />
						...
						<div className="basic-input">
							<div className="name-and-tagline">
								...
								<input
									...
									type={"text"}
									placeholder={"Enter your profile tagline *"}
								/>
							</div>
							<hr />
							<div className="social-media-links">
								...
								<div>
									<input
										value={portfolioLink}
										onInput={(e) => setPortfolioLink(e.target.value)}
						...
						<div className="input2">
							<textarea
								...
								placeholder={
									"Enter your about section (maximum 300 characters) *"
								}
								maxLength={"300"}
							></textarea>
						</div>
					...
					<div className="footer-section">
							...
							<Dialog open={openProjectForm} onClose={handleCloseProjectForm}>
								...
								<DialogContent>
									...
									<TextField
										...
										value={projectRoles}
										onInput={(e) => {
											setProjectRoles(e.target.value);
									...
									<FormGroup>
										<FormControlLabel
											control={
												<Switch
													checked={projectCollaborated}
													onChange={(e) => {
														setProjectCollaborated(e.target.checked);
													}}
												/>
											}
											label="Was it a Collaborative project?"
										/>
									</FormGroup>
									<Autocomplete
										onChange={(option) => {
										...
										multiple
										defaultValue={
											typeof projectTechStacks == "string"
												? []
												: projectTechStacks
										}
										id="tags-outlined"
						...
						>
							...
							<TextField
								size="small"
								style={{ marginTop: "16px" }}
								value={studentAccomplishment}
								onInput={(e) => setStudentAccomplishment(e.target.value)}
								inputProps={{ maxLength: "200" }}
								margin="dense"
								multiline
								rows={3}
								label={
									"Accomplisments, each in new line (maximum 150 characters)"
								}
								id="standard-basic"
								type="url"
								variant="outlined"
							...
							<TextField
								...
								onInput={(e) => setStudentInterests(e.target.value)}
								inputProps={{ maxLength: "100" }}
								margin="dense"
								multiline
								rows={2}
								label={
									"Interests, each separated by (,) (maximum 100 characters)"
								}
								id="standard-basic"
								type="url"
								variant="outlined"
					...
					<div style={{ textAlign: "center", "margin-top": "20px" }}>
						<FormControlLabel
							control={
								<Switch
									checked={showExperienceRendering}
									onChange={(e) => {
										setShowExperienceRendering(e.target.checked);
									}}
								/>
							}
							label="Do you want to add work experience in your resume?"
						/>
								...
								>
									...
									<DialogContent>
										...
										<TextField
											...
											margin="dense"
											label={
												"Roles in the company, each in new line (maximum 400 characters) *"
											}
											type="text"
											fullWidth
											multiline
