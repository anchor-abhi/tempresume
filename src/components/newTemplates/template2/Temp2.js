	...
	return load ? (
		<div className="Temp2">
			...
			<div className="section">
				<div className="">
					{data.workEx && data?.workEx?.length != 0 ? (
						<div className="edu">
							<Title cont="PROFESSIONAL EXPERIENCE"></Title>
							{data?.workEx?.map((el) => (
								<WorkEx
									name={el.organisation}
									start={el.start}
									end={el.end}
									pos={el.position}
					...
					<div>
						...
						<div className="accom">
							{data?.accomplishments?.map((el) => (
								<Accomplishments accom={el}></Accomplishments>
							))}
						</div>
					</div>
					<div>
				...
				<div className="">
					<div>
						...
						<div className="skills">
							{" "}
							{data?.techSkills?.map((el) => (
								<SkillsAndFrameworks skill={el}></SkillsAndFrameworks>
							))}
						</div>
					</div>
					<div>
