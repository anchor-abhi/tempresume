                     ...
                     <>
                        <div key={uuid()} id="left" style={{background: leftDivColor }}>
                           <div id="nameAndDesignation" style={{marginTop: leftSlider}}>
                              <h2 style={{color: headingColor}} id="nameH1">{one?.personal?.name}</h2>
                              <h5 style={{color: headingColor}} id="designation">{one?.personal?.tagLine}</h5>
                           </div>
                           <div id="aboutMePara">
                              <h4 style={{color: headingColor}}>About</h4>
                              <p style={{color: textColor}}>{one?.summary}</p>
                           </div>
                           {hasWork && <Education headingColor={headingColor} textColor={textColor} one={one}/>} 
                           ...
                           <div id="accomplishments">
                              <h4 style={{color: headingColor}}>Accomplishments</h4>
                              {
                                 one?.accomplishments?.map(oneacc => {
                                    return (
                                       <p key={uuid()} style={{color: textColor}} className="singleAcc">{oneacc}</p>
                                    )
                                 })
                              }
                           </div>
                           <img id="profileImg" src={one?.personal?.profilePic} style={{top: imgSlider}} alt="image" />
                           <div id="skillsAndFramework">
                              <h4 style={{color: headingColor}}>Skills and Frameworks</h4>
                              <span style={{color: textColor}}>{one?.techSkills.join(" | ")}</span>
                           </div>
                           <div id="softSkills">
                              <h4 style={{color: headingColor}}>Soft Skills</h4>
                              <span style={{color: textColor}}>{one?.softSkills?.join(" | ")}</span>
                           </div>

                           <div id="interests">
                              <h4 style={{color: headingColor}}>Interests</h4>
                              <p style={{color: textColor}}>{one?.interests?.join(" | ")}</p>
                           </div>
                        ...
                        <div id="right" >
                                    ...
                                    return (
                                       <div key={uuid()} className="singleProject">
                                             ...
                                             <ul>
                                                {
                                                   sproject?.features?.map(sfeature => {
                                                      return (
                                                         <li style={{color: textColor}} key={uuid()}>{sfeature}</li>
                                                      )
                                                   })   
                                                }
                                             </ul>
                                          ...
                                          <p style={{color: textColor}} className="techStack">
                                             <span style={{color: headingColor}} className="heading">Tech Stack:
                                             </span> {sproject?.techStack?.join(" | ")}
                                          </p>
                                          <div className="areasOfResp">
                                             <h5 style={{color: headingColor}}>Areas of Responsibilities</h5>
                                             <ul>
                                                {
                                                   sproject?.areasOfResp?.map(onearea => {
                                                      return (
                                                         <li style={{color: textColor}} key={uuid()}>{onearea}</li>
                                                      )
                                                   })
                                                }
                                             </ul>
                                          </div>  
                                          <p style={{color: textColor}} className="lastLine">{sproject?.solo ? "A solo project built in 7 days" : `A collaborative project built by team of ${sproject?.team}`}</p>
                                       </div>
                                    )
                                 })
                              }
                           </div> 
                           {
                                 ...
                                 <div id="workEx">
                                    <h4>Work Experience</h4>
                                    {
                                       one?.workEx?.map(onework => {
                                          return (
                                             <>
                                                <p className="orgAndPos">{onework?.organisation} - <span>{onework?.position}</span></p>
                                                <p className="workDuration">{onework?.start} - {onework?.end}</p>
                              ...
                              ) : <Education headingColor={headingColor} textColor={textColor} one={one}/> 
